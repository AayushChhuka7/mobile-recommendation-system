...
export const getRecommendations = async (body, userId, opts = {}) => {
  ...
  // CF hybrid merge — union CF candidates into the ranked list. CF
  // rows that match an existing ML row only contribute a `cfReasons`
  // hint; CF-only rows are appended after the ranked list so they
  // always appear below the personal picks but above the (already
  // empty) tail. Failures degrade silently — empty cfPayload → no-op.
  const { merged: cfMerged } = await mergeCfCandidates(ranked, cfPayload);
  // Fire-and-forget analytics. Logs the served CF list and the user's
  // cluster so future segmentation re-trains have a feedback loop.
  if (userId && cfPayload && Array.isArray(cfPayload.results) && cfPayload.results.length > 0) {
  ...
  // "Boosted by your activity" badge). CF-only candidates (no
  // `finalScore` because they skipped the ranker) keep their
  // CF-derived matchScore so the percentage UI still renders.
  const finalRanked = cfMerged.map((c) => {
    if (c.finalScore != null && Number.isFinite(c.finalScore)) {
      return {
        ...c,
  ...
  // because the ranked CF merge keeps ML rows above CF-only rows and
  // `fusionRank` returned ML rows in `finalScore` desc order — the
  // first occurrence is always the highest-ranked row for each
  // identity.
...
export const getRecommendationsTwoStage = async (body, userId, opts = {}) => {
  ...
  //
  // CF candidates are also fetched in parallel (see `mlResults` below).
  // CF failures degrade silently to an empty envelope.
  const [data, cfPayload] = await Promise.all([
    mlFetch("/recommend", {
      method: "POST",
      body: JSON.stringify({
